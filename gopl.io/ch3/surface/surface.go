...
func handler(w http.ResponseWriter, r *http.Request) {
	...
	for i := 0; i < cells; i++ {
		for j := 0; j < cells; j++ {
			...
			fmt.Fprintf(w, "<polygon points='%g,%g %g,%g %g,%g %g,%g' style='fill:"+color+";'/>\n",
...
func getColor(num float64) string {
	scale := (1 + num) / 2.0
	red := uint(float64(0xff) * scale)
	green := 0x00
	blue := uint(float64(0xff) * (1 - scale))
